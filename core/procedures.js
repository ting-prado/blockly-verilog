...
Blockly.Procedures.allProcedures = function (root) {
  var proceduresNoReturn = root
    .getBlocksByType('procedures_defnoreturn', false)
    .map(function (block) {
      return /** @type {!Blockly.Procedures.ProcedureBlock} */ (
        block
      ).getProcedureDef();
    });
  var proceduresReturn = root
...
Blockly.Procedures.rename = function (name) {
  ...
  var legalName = Blockly.Procedures.findLegalName(
    name,
...
Blockly.Procedures.mutatorOpenListener = function (e) {
  if (
    !(
      e.type == Blockly.Events.BUBBLE_OPEN &&
      e.bubbleType === 'mutator' &&
      e.isOpen
    )
  ) {
    return;
  ...
  var block = Blockly.Workspace.getById(workspaceId).getBlockById(e.blockId);
  var type = block.type;
