...
function maybeAddClosureLibrary(srcs) {
  if (argv.closureLibrary) {
    ...
    if (!fs.existsSync('./node_modules/google-closure-library')) {
      throw Error(
        'You must add the google-closure-library to your ' +
...
function buildCompressed() {
  ...
  return (
    gulp
      .src(maybeAddClosureLibrary(['core/**/**/*.js']), { base: './' })
      ...
      .pipe(
        gulp.sourcemaps.mapSources(function (sourcePath, file) {
...
function buildGenerator(language, namespace) {
  return gulp
    .src([`generators/${language}.js`, `generators/${language}/*.js`], {
      base: './',
    })
    .pipe(stripApacheLicense())
    .pipe(gulp.sourcemaps.init())
      ...
      compile(
        {
          dependency_mode: 'NONE',
          externs: [
            './externs/goog-externs.js',
            './externs/generator-externs.js',
          ],
...
`;
  ...
  let deps = [];
  return gulp
    .src(maybeAddClosureLibrary(['core/**/**/*.js']))
    .pipe(
...
`;
      fs.writeFileSync(
        'blockly_uncompressed.js',
        header + addDependency + requires + footer
      );
    });
}
...
function buildAdvancedCompilationTest() {
  ...
  return (
    gulp
      .src(maybeAddClosureLibrary(srcs), { base: './' })
      .pipe(stripApacheLicense())
      .pipe(gulp.sourcemaps.init())
      // Directories in Blockly are used to group similar files together
      // but are not used to limit access with @package, instead the
 ...
 *     msg/json/*.js
 */
const build = gulp.parallel(buildCore, buildGenerators, buildLangfiles);
