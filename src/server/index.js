...
Promise.resolve(
  (async () => {
    ...
    app.post(
      'blockly-verilog.herokuapp.com/api/yosys2digitaljs',
      async (req, res) => {
        try {
    ...
    app.post(
      'blockly-verilog.herokuapp.com/api/storeCircuit',
      async (req, res) => {
        try {
          ...
          const stmt = await db.run(
            SQL`UPDATE storedCircuits SET lastAccess=datetime('now') WHERE hash=${hash}`
          );
          if (stmt.changes == 0) {
            await db.run(
              SQL`INSERT INTO storedCircuits(hash, json, date, lastAccess) VALUES(${hash}, ${sdata}, datetime('now'), datetime('now'))`
            );
          }
    ...
    app.get(
      'blockly-verilog.herokuapp.com/api/circuit/:hash',
      async (req, res) => {
        try {
          const data = await db.get(
            SQL`SELECT json FROM storedCircuits WHERE hash=${req.params.hash}`
          );
          if (data === undefined)
